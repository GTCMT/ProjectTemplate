    ...
    m_iNumChannels(iNumChannels)
{
    ...
    for (int i = 0; i < CMyProject::kNumFilterParams; i++)
    {
        m_afParam[i]            = 0.F;
    }
    m_aafParamRange[CMyProject::kParamGain][0]   = std::numeric_limits<float>::min();
    m_aafParamRange[CMyProject::kParamGain][1]   = std::numeric_limits<float>::max();
    m_aafParamRange[CMyProject::kParamDelay][0] = 0;
    m_aafParamRange[CMyProject::kParamDelay][1] = static_cast<float>(iMaxDelayInFrames);
...
Error_t CCombFilterBase::resetInstance()
{
    for (int c = 0; c < m_iNumChannels; c++)
    {
        m_ppCRingBuffer[c]->resetInstance();
        m_ppCRingBuffer[c]->setWriteIdx(CUtil::float2int<int>(m_afParam[CMyProject::kParamDelay]));
    }

    return kNoError;
}

Error_t CCombFilterBase::setParam( CMyProject::FilterParam_t eParam, float fParamValue )
{
    if (isInParamRange(eParam, fParamValue))
        return kFunctionInvalidArgsError;

    // special actions for special parameters
    if (eParam == CMyProject::kParamDelay)
    {
        int iNumAdditionalTaps  = CUtil::float2int<int>(fParamValue - m_afParam[CMyProject::kParamDelay]);
...
float CCombFilterBase::getParam( CMyProject::FilterParam_t eParam ) const
{
    return m_afParam[eParam];
}

bool CCombFilterBase::isInParamRange( CMyProject::FilterParam_t eParam, float fValue )
{
...
Error_t CCombFilterFir::process( float **ppfInputBuffer, float **ppfOutputBuffer, int iNumberOfFrames )
{
    for(int c = 0; c < m_iNumChannels; c++)
    {
        for (int i = 0; i < iNumberOfFrames; i++)
        {
            ...
            ppfOutputBuffer[c][i] = ppfInputBuffer[c][i] + m_afParam[CMyProject::kParamGain] * m_ppCRingBuffer[c]->getPostInc();
        }
    }
    return kNoError;
}
...
{
    // set limits
    m_aafParamRange[CMyProject::kParamGain][0] = -1.F; 
    m_aafParamRange[CMyProject::kParamGain][0] = 1.F;
}
